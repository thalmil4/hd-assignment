...
├── Makefile                                   # Docker test automation commands
...
│   │   │   └── SecurityTestRunner.java       # Run security tests only
│   │   └── utils/
...
```
make test ARGS='-Dtest=ValidTestRunner'
make test ARGS='-Dtest=InvalidTestRunner'
make test ARGS='-Dtest=SecurityTestRunner'
make test ARGS='-Dtest=RegressionTestRunner'
```
